...
- `src/generator.ts` — 選ばれたプリセットのエージェント定義を `{{placeholder}}` 置換して対象リポジトリの `.claude/agents/` に書き込み、`.claude/team.json` にマニフェスト(`TeamManifest`)を記録。各エージェント定義の末尾に「完了時に `.claude/atf-logs/runs.jsonl` へ実行記録を追記する」指示を自動付与する。実行環境の整備役 `templates/common/env-builder.md`(ハーネス・ガードレール・フィードバックループの設定とコンテキストファイル整備を担当)と、チーム全体をまとめ上げて開発を推進する `templates/common/orchestrator.md`(2 つの動作モードを持つ: 既定は立ち上げ期向けの**ブートストラップモード** = Issue/PR を扱わず作業単位ごとにコミット & push、もう一方が **Issue 駆動モード** = Issue 起点のループ。タッチポイントに応じた「一時停止してユーザー承認を仰ぐ」指示は orchestrator にのみ generator が付与する)は全チーム共通で teamSize の枠外で追加する。flow には orchestrator → 入口エージェント(Issue 駆動なら issue-manager、それ以外はチーム先頭)の辺が加わる。Issue 駆動(`requirements.issueDriven`)なら `templates/common/issue-manager.md` を teamSize の枠外で追加し、全エージェントに Issue 起点で動く指示を付与する。PR フロー(`requirements.prFlow`)なら全エージェントにブランチ作成 → push → `gh pr create` → マージの手順を付与し、タッチポイント(`requirements.touchpoints`)に応じてマージの実行主体が変わる(`pr-merge` あり = ユーザーがマージ、エージェントの `gh pr merge` を禁止 / なし = CI・レビュー確認後にエージェントがマージ)。`issue-approval` は Issue 承認後に着手する指示を追加する
...
`templates/presets/<id>/` に `preset.json` と `agents/*.md` を置くだけでコード変更なしで認識される。全プリセット共通のエージェント(env-builder / orchestrator / issue-manager など)は `templates/common/` に置く。
