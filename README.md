...
対象リポジトリを自動解析し、対話ヒアリング(開発フェーズ・重視観点・チーム規模・Issue 駆動開発の有無)で要件を把握したうえで、最適なチームプリセットを選定・カスタマイズして、対象リポジトリの `.claude/agents/` に Claude Code サブエージェント定義一式を導入します。新規開発では、目的・対象ユーザー・中核価値・スコープ・成功基準・リスク・トレードオフを追加でヒアリングし、`.claude/inception-deck.md` にインセプションデッキとして保存します。
